...
class NotificationTypeConfig:
    ...
    email_template: Optional[str] = None  # EmailTemplate.name
    push_template: Optional[str] = None  # Push notification template
    sms_template: Optional[str] = None  # SMS template
...
NOTIFICATION_TYPES = {
    ...
    'follow_request_received': NotificationTypeConfig(
        ...
        required_context=['transaction_id', 'follower_id', 'followee_type', 'followee_id'],
        user_configurable=True,
    ),
...
def get_notification_type(name: str) -> Optional[NotificationTypeConfig]:
