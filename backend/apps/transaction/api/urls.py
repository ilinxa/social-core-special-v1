...
urlpatterns = [
    ...
    path(
        "types/<str:transaction_type>/form/",
        TransactionFormSchemaView.as_view(),
        name="form-schema",
    ),
    path(
        "form-mappings/check/",
        RequestFormCheckView.as_view(),
        name="request-form-check",
    ),
    path(
        "form-mappings/",
        TransactionFormMappingListCreateView.as_view(),
        name="form-mapping-list-create",
    ),
    path(
        "form-mappings/<uuid:mapping_id>/",
        TransactionFormMappingDeleteView.as_view(),
        name="form-mapping-delete",
    ),
    path("<uuid:transaction_id>/", TransactionDetailView.as_view(), name="detail"),
    path(
        "<uuid:transaction_id>/accept/", AcceptTransactionView.as_view(), name="accept"
    ),
    path(
        "<uuid:transaction_id>/approve/",
        ApproveTransactionReviewView.as_view(),
        name="approve",
    ),
    path("<uuid:transaction_id>/deny/", DenyTransactionView.as_view(), name="deny"),
    path(
        "<uuid:transaction_id>/cancel/", CancelTransactionView.as_view(), name="cancel"
    ),
    path(
        "<uuid:transaction_id>/dismiss/",
        DismissTransactionView.as_view(),
        name="dismiss",
    ),
    path(
        "<uuid:transaction_id>/request-info/",
        TransactionRequestInfoView.as_view(),
        name="request-info",
    ),
    path(
        "<uuid:transaction_id>/resubmit/",
        TransactionResubmitView.as_view(),
        name="resubmit",
    ),
    path(
        "<uuid:transaction_id>/form-response/",
        TransactionFormResponseView.as_view(),
        name="form-response",
    ),
    path(
        "<uuid:transaction_id>/required-form/",
        TransactionRequiredFormView.as_view(),
        name="required-form",
    ),
]
