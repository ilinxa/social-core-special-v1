...
class TestFollowServiceRemoveFollower:
    ...
    def test_remove_follower_success(self, business, immediate_on_commit):
        """Owner can remove a follower; status becomes REMOVED and notification fires."""
        from unittest.mock import patch

        from apps.core.types import ActorContext

        owner = business.created_by
        follower = UserFactory(is_verified=True)
        follow = FollowFactory(
            follower=follower,
            followee_type="business",
        ...
        actor_context = ActorContext.for_user_context(owner)

        with patch("apps.notifications.services.NotificationService.send") as mock_send:
            result = FollowService.remove_follower(
                follow_id=follow.id,
                actor=owner,
                actor_context=actor_context,
            )
        ...
        assert result.status == FollowStatus.REMOVED
        assert result.removed_at is not None
        assert result.removed_by == owner
        mock_send.assert_called_once()
        call_kwargs = mock_send.call_args
        assert call_kwargs[1]["user"] == follower

    def test_remove_follower_already_removed_raises(self, business):
        """Cannot remove an already-removed follower."""
        from apps.core.types import ActorContext
