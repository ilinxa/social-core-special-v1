...
> **v2.1 Changes:**
> - Fixed: Added `ValidationError` to service import statement
> - Fixed: Removed dead `clean()` from RolePermission (service-layer validates scope)
> - Fixed: PermissionSelector returns `[]` for non-active memberships instead of raising DoesNotExist
> - Fixed: Complete permission seeding implementation for `initialize_business_account`
> - Fixed: Complete `initialize_platform_account` implementation
> - Fixed: `MembershipManager` now inherits `SoftDeleteManager`
> - Fixed: `transfer_ownership` documented as stub (deferred to Transaction system)
>
...
class RBACService:
    ...
    def initialize_business_account(*, business_id: UUID, owner, request=None) -> Membership:
        ...
        business_permissions = Permission.objects.filter(
            applicable_scopes__contains=["business"]
        )
    ...
    def initialize_platform_account(*, platform_id: UUID) -> None:
        ...
        admin_perms = Permission.objects.filter(
            applicable_scopes__contains=["platform_only"]
        )
        RolePermission.objects.bulk_create([
            RolePermission(
                role=admin_role,
        ...
        global_perms = Permission.objects.filter(
            applicable_scopes__contains=["global_only"]
        )
        RolePermission.objects.bulk_create([
            RolePermission(
                role=mod_role,
    ...
    def transfer_ownership(
        *, account_type: str, account_id: UUID,
        new_owner, transferred_by=None, request=None
    ) -> tuple:
        """
        Transfer ownership of an account to a new owner.

        STUB: Full implementation deferred to Transaction system (System 4).
        The Transaction system's OwnershipTransferOutcomeHandler will call this
        method when the new owner accepts the transfer request.

        Implementation must (in a single atomic transaction):
        1. Get current owner membership (is_owner=True for this account)
        2. Get new owner's existing membership (must already be a member)
        3. Set old owner: is_owner=False, assign a non-owner role (e.g., Admin or custom)
        4. Set new owner: is_owner=True, assign the Owner role (level 0)
        5. Invalidate cached permissions for BOTH memberships
        6. Audit log: OWNERSHIP_TRANSFERRED with old_owner and new_owner details

        Args:
            account_type: AccountType value
            account_id: UUID of the account
            new_owner: User who will become the new owner
            transferred_by: User who initiated (may be the old owner or the new owner accepting)
        ...
        raise NotImplementedError(
            "transfer_ownership requires the Transaction system. "
            "See OwnershipTransferOutcomeHandler."
        )
